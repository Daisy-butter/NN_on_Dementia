...
from torchvision.models.video import r3d_18  # 用ResNet结构的3D模型
...
# 新网络结构：升级为基于ResNet的3D CNN
class ResNet3D(nn.Module):
    def __init__(self):
        super(ResNet3D, self).__init__()

        # 使用 ResNet 的 3D 变体
        self.cnn = r3d_18(pretrained=False)  # 可以将 `pretrained=True` 以加载预训练权重
        self.cnn.fc = nn.Identity()  # 移除最后一层全连接层

        # 添加新的分类头
        self.fc = nn.Sequential(
            nn.Linear(512 + 2, 256),  # 512 是 ResNet 输出特征的维度，2 是 age 和 gender 的输入
            nn.ReLU(),
            nn.Linear(256, 3)  # 最后输出 3 类
        )
